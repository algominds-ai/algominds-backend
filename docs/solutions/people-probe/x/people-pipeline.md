...
flowchart TD
    ...
    subgraph company["one company"]
        ...
        S5["5 · VERIFY — one Exa agent run, structured output<br/>{verdict, evidence_url, evidence_kind, quote, hook}<br/>first-party / press evidence stands alone<br/>aggregator-only → people index as second opinion<br/>silence → <b>unknown, excluded</b>"]
...
| Exa agent, effort minimal | verification with structured output: verdict, first-party evidence, hook — $0.012, ~20 s | retrieval ($0.225 for 4 people already in the roster) |
