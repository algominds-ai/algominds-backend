...
export const REQUIREMENTS_INSTRUCTIONS = [
	...
	"`proof` is `record` when a structured company record can establish it, such as the",
	"headcount, the country, the founded year, the revenue, or the industry as described;",
	"`page` when only a public page can, such as a technology in production, a",
	"certification, a regulatory status, or a hiring event. A test settled only by finding",
	"nothing is `record`, never `page`.",
