...
const SYNTHESIZE_INSTRUCTIONS = [
	...
	"date, but it takes minutes rather than seconds. Choose `exa-agent` whenever this round",
	"asks for an event a page has to prove, whether or not you demand that the page be",
	"fresh: a profile naming a breach, a licence or a funding round is asking for evidence,",
	"and the index holds none, so a round on it comes back with nothing a judge can keep.",
	"Choose `exa-search` only when the round asks about lasting shape alone, such as a size,",
	"a country or an industry, and names no event at all.",
