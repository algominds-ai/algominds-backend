...
## Appendix B. The frozen regression set

`eval/regression/judge-rows.json` on `p2/base` (commit 759b990): 55 judged company rows with the exact judge input (row fields, page evidence, requirements), the prior judge statuses where a judge-output row existed, and the key ruling as `expected` (accept or reject): form3 13 accept and 4 reject, ondato 19 accept and 19 reject. Produced by `eval/regression-build.ts` from every local `eval_p2*` database that holds judge-input rows. Use: replay these rows through `judge()` after any judge change (the `--captured` plumbing in `eval/judge-replay.ts` rebuilds a call from a judge-input row) and report kept/refused against `expected` per slug before any live run. The replay mode itself was not finished in this round; the data is the deliverable.
